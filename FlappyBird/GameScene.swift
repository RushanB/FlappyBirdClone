...
//    weak var gameSceneDelegate: GameSceneDelegate?
    ...
    //added scoreboard node
    var scoreboard:SKSpriteNode!
    //added restart Label Node
    var restartLabelNode:SKLabelNode!
    //added share label Node
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if moving.speed > 0  {
            for touch: AnyObject in touches { // do we need all touches?
            ...
            //Implemented restart methods
        } else if canRestart == true {
            for touch: AnyObject in touches{
                let location = touch.location(in: self)
                
                if self.atPoint(location) == restartLabelNode{
                    self.scoreLabelNode.isHidden = true
                    self.restartLabelNode.isHidden = true
//                    self.scoreboard.hidden = true
        ...
        //share the snapshot via UIActivityViewC
        if self.atPoint(location) == scoreLabelNode{
            let snapshotImage: UIImage = snapshot(view: self.view!)
    ...
    func showScoreboard() {
        ...
        //print("Game Over")
        self.shareLabelNode.isHidden = false
        self.restartLabelNode.isHidden = false
                ...
                self.run(SKAction.sequence([SKAction.repeat(SKAction.sequence([SKAction.run({
                    self.backgroundColor = SKColor(red: 1, green: 0, blue: 0, alpha: 1.0)
                        ...
                        self.backgroundColor = self.skyColor
                            ...
                            //start to inject my kill screen, show the buttons
//                            self.gameSceneDelegate?.showButtons()
                            })]), withKey: "flash")
